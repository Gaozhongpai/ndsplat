...
class GaussianModel:
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate(
            (xyz, f_dc, f_rest, opacities, betas, mean, scale, l_triangle),
            axis=1,
        )
    ...
    def load_ply(self, path):
        ...
        self._l_triangle = nn.Parameter(
            torch.tensor(l_triangles, dtype=torch.float, device="cuda").requires_grad_(True)
        )
    ...
    def save_png(self, path):
        ...
        param_dict = {
            ...
            "f_rest": self._features_rest.transpose(1, 2).flatten(start_dim=1).contiguous(),
    ...
    def load_png(self, path):
        ...
        f_dc_flat = decompress_png(path, "f_dc", meta["f_dc"])
        f_rest_flat = decompress_png(path, "f_rest", meta["f_rest"])
        # Reshape features back to (N, 3, SH_coeffs) format
        f_dc = f_dc_flat.reshape((f_dc_flat.shape[0], 3, 1))
        f_rest = f_rest_flat.reshape((f_rest_flat.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1))
        ...
        self._features_dc = nn.Parameter(
            torch.tensor(f_dc, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True)
        )
        self._features_rest = nn.Parameter(
            torch.tensor(f_rest, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True)
        )
        self._opacity = nn.Parameter(
        ...
        self._l_triangle = nn.Parameter(
            torch.tensor(l_triangle, dtype=torch.float, device="cuda").requires_grad_(True)
        )
