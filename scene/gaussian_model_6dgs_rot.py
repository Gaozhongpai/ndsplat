...
class GaussianModel:
    ...
    def densify_and_clone_split_v2(self, grads, grad_threshold, scene_extent, rotation, scale, N=3):
        ...
        # Merging new_scale_split and new_scale_clone
        new_scale = self._scale[selected_pts_mask].repeat(N, 1)        
        # split_diags = self.scale_activation_inv(self.scale_activation(self._scale[selected_pts_mask_split]) * 2/3)
        # new_scale[selected_pts_mask_split[selected_pts_mask]] = split_diags
        # new_scale = new_scale.repeat(N, 1)
    ...
    def densify_and_split_6d(self, grads, grad_threshold, scene_extent, rotation, scale, N=2):
        ...
        new_features_rest = self._features_rest[selected_pts_mask].repeat(N,1,1)
        new_opacity = self._opacity[selected_pts_mask].repeat(N,1)
        new_scale = self.scale_activation_inv(self.scale_activation(self._scale[selected_pts_mask]) * 0.8).repeat(N, 1)
        # new_scale = self._scale[selected_pts_mask].repeat(N, 1)
        new_l_triangle = self._l_triangle[selected_pts_mask].repeat(N, 1)
