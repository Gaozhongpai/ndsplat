...
# Import gsplat functions for N-DGS operations (v2)
from gsplat import (
    slice_gaussian_ndgs_v2,
    l_triangle_to_covar,
...
class GaussianModel:
    ...
    def slice_gaussian(self, q, c_dim=3, lambda_view=None, lambda_time=None):
        """
        Perform conditional Gaussian slicing for N-DGS v2.
        Given ND Gaussian with mean [m_1, m_2] and covariance v,
        compute conditional distribution given observation q for m_2.

        Uses CUDA-accelerated gsplat v2 implementation with lambda interpolation.

        Args:
            q: Query direction (view direction + time for 7DGS) [N, C]
            c_dim: Conditional dimension (default 3 for spatial xyz)
            lambda_view: Lambda for view direction V_22_inv interpolation. Tensor [N] or scalar.
                        If None, uses self.get_lambda_view (default behavior)
            lambda_time: Lambda for time V_22_inv interpolation. Tensor [N], scalar, or None.
                        If None, uses self.get_lambda_time for 7DGS (default behavior)

        Returns:
            m_cond: Conditional mean (3D position) [N, 3]
            cov3D_precomp: Base covariance v_11 (upper triangular elements) [N, 6]
        ...
        # When use_view_dependent_pos=False, lambda parameters are not needed for position shifting
        # Pass default scalar values (CUDA kernel won't use them, but wrapper needs valid inputs)
        if not self.use_view_dependent_pos:
            lambda_view = self.default_lambda_view  # Use default scalar value
            lambda_time = None  # Will default to lambda_view in CUDA wrapper

        # Use CUDA-accelerated conditional Gaussian slicing v2
        m_cond, cov3D_precomp, scale = slice_gaussian_ndgs_v2(
            m_1=m_1,
            m_2=m_2,
            query=q,
            covars=v,
            use_view_dependent_pos=self.use_view_dependent_pos,
            lambda_view=lambda_view,
            lambda_time=lambda_time,
            lambda_opc=self.lambda_opc,
    ...
    def __init__(self, sh_degree : int, input_dim: int = 6, use_rot_scale_l_triangle: bool = True,
                 learnable_lambda_view: bool = True, time_duration: list = [0.0, 1.0], lambda_view: float = 0.5,
                 use_view_dependent_pos: bool = True, lambda_opc: float = 0.35):
        """
        Initialize GaussianModel v2 with flexible covariance parametrization.
        ...
        Args:
            ...
            learnable_lambda_view: If True, make lambda_view a learnable parameter per Gaussian.
            time_duration: [min, max] time range for 7DGS (default: [0.0, 1.0]).
            lambda_view: Default lambda_view value for V_22_inv interpolation (default: 0.5).
            use_view_dependent_pos: If True, enable view-dependent position shift (default: True).
        """
        ...
        # Store default lambda_view value
        self.default_lambda_view = lambda_view

        # Track lambda_view training state to avoid redundant enable/disable calls
        self._lambda_view_training_enabled = False
        self.learnable_lambda_view = learnable_lambda_view
        self.time_duration = time_duration  # Time range for 7DGS
        self.use_view_dependent_pos = use_view_dependent_pos  # V2: optional position shift
        self.lambda_opc = lambda_opc  # Lambda for opacity scaling
        ...
        # Only initialize lambda parameters if view-dependent position shift is enabled
        if self.use_view_dependent_pos:
            self._lambda_view = torch.empty(0)  # Learnable lambda for view V_22_inv interpolation
            self._lambda_time = torch.empty(0)  # Learnable lambda for time V_22_inv interpolation (only for input_dim=7)
        else:
            self._lambda_view = None
            self._lambda_time = None
    ...
    @property
    def get_lambda_view(self):
        """Get lambda_view parameter for view V_22_inv interpolation (learnable or fixed)."""
        if not self.use_view_dependent_pos:
            return None  # Not needed when position shift is disabled
        if self.learnable_lambda_view:
            return self.opacity_activation(self._lambda_view)
        else:
            # Return fixed default value
            return torch.ones_like(self._opacity) * self.default_lambda_view

    @property
    def get_lambda_time(self):
        """Get lambda_time parameter for time V_22_inv interpolation (learnable or fixed, only for input_dim=7)."""
        if not self.use_view_dependent_pos:
            return None  # Not needed when position shift is disabled
        if self.input_dim != 7:
            return None
        if self.learnable_lambda_view:
            return self.opacity_activation(self._lambda_time)
        else:
            # Return fixed default value for time component
            return torch.ones_like(self._opacity) * self.default_lambda_view

    def _compute_conditional_spatial_covariance(self, v: torch.Tensor) -> torch.Tensor:
        """
        Returns the marginal spatial covariance v_11 directly.

        Args:
            v: Full covariance matrix [N, D, D]

        Returns:
            Marginal spatial covariance v_11 [N, 3, 3]
        """
        # Always return v_11 (marginal spatial covariance)
        return v[:, :3, :3]
    ...
    def get_scaling(self):
        ...
        # V2: Use conditional spatial covariance (v_cond = v_11 - v_regr @ v_21)
        # This matches the CUDA implementation which returns the Schur complement
        v_cond = self._compute_conditional_spatial_covariance(v)
    ...
    def get_rotation_scale(self):
        """
        Get rotation and scale from covariance.

        For 7DGS (input_dim=7), also returns sigma_pt (position-time correlation).
        V2: Uses conditional spatial covariance (v_cond) from Schur complement.
        ...
        # For 7DGS, extract position-time correlation for time-based splitting
        sigma_pt = v[:, :3, 6:7] if self.input_dim == 7 else None
    ...
    def get_xyz_covariance(self):
        ...
        V2: Returns the conditional spatial covariance (v_cond = v_11 - v_regr @ v_21).
        This is used for MCMC densification to add covariance-weighted spatial noise.
        ...
        if self.use_rot_scale_l_triangle:
            ...
            # V2: Use conditional spatial covariance (v_cond from Schur complement)
            v = self.get_pc_v
            return self._compute_conditional_spatial_covariance(v)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, mcmc_cap_max=None, densification_strategy="standard"):
        ...
        # Initialize lambda_view for view
        lambda_views = inverse_sigmoid(self.default_lambda_view * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))

        # Initialize lambda_time for time (only for 7DGS)
        if self.input_dim == 7:
            lambda_views_time = inverse_sigmoid(self.default_lambda_view * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))
        else:
            lambda_views_time = torch.empty((fused_point_cloud.shape[0], 0), dtype=torch.float, device="cuda")
        ...
        # lambda_view/time parameters: only needed if view-dependent position shift is enabled
        if self.use_view_dependent_pos:
            # lambda_view parameters: initially non-trainable, enabled via patient-based training
            # Similar to opacity_scale in 4D-GS: start frozen, enable after patience or iteration > 14900
            self._lambda_view = nn.Parameter(lambda_views)
            self._lambda_view.requires_grad_(False)  # Start frozen
            self._lambda_time = nn.Parameter(lambda_views_time)
            self._lambda_time.requires_grad_(False)  # Start frozen
    ...
    def training_setup(self, training_args):
        ...
        # Add lambda_view parameter (only if view-dependent position shift is enabled)
        if self.use_view_dependent_pos:
            ...
            if self.learnable_lambda_view:
                l.append({'params': [self._lambda_view], 'lr': 0.0, "name": "lambda_view"})  # Start frozen
            else:
                l.append({'params': [self._lambda_view], 'lr': 0.0, "name": "lambda_view"})

            # Add lambda_time parameter (only for input_dim=7)
            if self.input_dim == 7:
                if self.learnable_lambda_view:
                    l.append({'params': [self._lambda_time], 'lr': 0.0, "name": "lambda_time"})  # Start frozen
                else:
                    l.append({'params': [self._lambda_time], 'lr': 0.0, "name": "lambda_time"})
    ...
    def enable_lambda_view_training(self, training_args):
        """
        Enable training for lambda_view parameters (patient-based training).
        Similar to opacity_scale in 4D-GS: triggered after patience or iteration > 14900.
        Only applies when learnable_lambda_view=True.

        Returns:
            bool: True if state changed, False if already enabled
        """
        if not self.learnable_lambda_view or not self.use_view_dependent_pos:
            return False

        # Check if already enabled to avoid redundant work
        if self._lambda_view_training_enabled:
            return False

        # Enable gradient computation
        self._lambda_view.requires_grad_(True)
        if self.input_dim == 7:
            self._lambda_time.requires_grad_(True)

        # Update optimizer learning rates
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "lambda_view":
                param_group['lr'] = training_args.opacity_lr
            elif param_group["name"] == "lambda_time" and self.input_dim == 7:
                param_group['lr'] = training_args.opacity_lr

        # Mark as enabled
        self._lambda_view_training_enabled = True
        return True

    def disable_lambda_view_training(self):
        """
        Disable training for lambda_view parameters.
        ...
        if not self._lambda_view_training_enabled:
            return False

        # Disable gradient computation
        self._lambda_view.requires_grad_(False)
        if self.input_dim == 7:
            self._lambda_time.requires_grad_(False)

        # Set optimizer learning rates to 0
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "lambda_view":
                param_group['lr'] = 0.0
            elif param_group["name"] == "lambda_time":
                param_group['lr'] = 0.0
    ...
    def construct_list_of_attributes(self):
        ...
        # Add lambda parameters only if view-dependent position shift is enabled
        if self.use_view_dependent_pos:
            l.append('lambda_view')
            # Add lambda_time only for 7DGS
            if self.input_dim == 7:
    ...
    def save_ply(self, path):
        ...
        attr_list = [xyz, mean_view, mean_time, f_dc, f_rest, opacities]

        # Add lambda parameters only if view-dependent position shift is enabled
        if self.use_view_dependent_pos:
            lambda_views = self._lambda_view.detach().cpu().numpy()
            attr_list.append(lambda_views)

            # Add lambda_time only for 7DGS
            if self.input_dim == 7:
                lambda_views_time = self._lambda_time.detach().cpu().numpy()
                attr_list.append(lambda_views_time)
    ...
    def load_ply(self, path):
        ...
        # Load lambda parameters (only if view-dependent position shift is enabled)
        if self.use_view_dependent_pos:
            # Load lambda_view (with backward compatibility)
            try:
                lambda_views = np.asarray(plydata.elements[0]["lambda_view"])[..., np.newaxis]
            except:
                # Default if not present in file
                lambda_views = np.full((xyz.shape[0], 1), inverse_sigmoid(self.default_lambda_view), dtype=np.float32)
            lambda_views = np.ascontiguousarray(lambda_views, dtype=np.float32)

            # Load lambda_time (only for 7DGS, with backward compatibility)
            if self.input_dim == 7:
                try:
                    lambda_views_time = np.asarray(plydata.elements[0]["lambda_time"])[..., np.newaxis]
                except:
                    # Default if not present in file
                    lambda_views_time = np.full((xyz.shape[0], 1), inverse_sigmoid(self.default_lambda_view), dtype=np.float32)
                lambda_views_time = np.ascontiguousarray(lambda_views_time, dtype=np.float32)
            else:
                lambda_views_time = np.empty((xyz.shape[0], 0), dtype=np.float32)
        else:
            # Not needed when use_view_dependent_pos=False
            lambda_views = None
            lambda_views_time = None
        ...
        # Only create lambda parameters if view-dependent position shift is enabled
        if self.use_view_dependent_pos:
            self._lambda_view = nn.Parameter(torch.tensor(lambda_views, dtype=torch.float, device="cuda").requires_grad_(self.learnable_lambda_view))
            self._lambda_time = nn.Parameter(torch.tensor(lambda_views_time, dtype=torch.float, device="cuda").requires_grad_(self.learnable_lambda_view and self.input_dim == 7))
    ...
    def prune_points(self, mask):
        ...
        # Update lambda parameters only if view-dependent position shift is enabled
        if self.use_view_dependent_pos:
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_mean_view, new_mean_time,
                                    new_opacities, new_lambda_view, new_scale, new_l_triangle, new_lambda_time=None):
        """
        ...
        Args:
            ...
            new_lambda_view: New lambda_view parameters
            new_lambda_time: New lambda_time parameters (only for 7DGS)
        ...
        # Add lambda parameters only if view-dependent position shift is enabled
        if self.use_view_dependent_pos and new_lambda_view is not None:
            d["lambda_view"] = new_lambda_view

            # Add lambda_time only for 7DGS
            if self.input_dim == 7 and new_lambda_time is not None:
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, rotation, scale, sigma_pt=None, N=2):
        ...
        new_lambda_view = self._lambda_view[selected_pts_mask].repeat(N,1) if self.use_view_dependent_pos else None
        new_lambda_time = self._lambda_time[selected_pts_mask].repeat(N,1) if (self.use_view_dependent_pos and self.input_dim == 7) else None
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_mean_view, new_mean_time,
                                   new_opacity, new_lambda_view, new_scale, new_l_triangle, new_lambda_time)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_mean_view, new_mean_time,
                                   new_opacities, new_lambda_view, new_scale, new_l_triangle, new_lambda_time)
    ...
    def _update_params(self, idxs, ratio):
        ...
        return (
            ...
            new_opacity,
            self._lambda_view[idxs] if self.use_view_dependent_pos else None,
            self._lambda_time[idxs] if (self.use_view_dependent_pos and self.input_dim == 7) else None,
            self._scale[idxs],
    ...
    def relocate_gs(self, dead_mask=None):
        ...
        (
            ...
            relocated_mean_time,
            relocated_opacity,
            relocated_lambda_view,
            relocated_lambda_time,
            relocated_scale,
        ...
        self._lambda_view.index_copy_(0, dead_indices, relocated_lambda_view)
        if self.input_dim == 7 and relocated_lambda_time is not None:
            self._lambda_time.index_copy_(0, dead_indices, relocated_lambda_time)
    ...
    def render_tcgs(self, viewpoint_camera, render_mode="RGB", scaling_modifier=1.0, use_tcgs=False, tight_snugbox=False, compact_box_mult=1.0):
        ...
        # Get lambda_view and lambda_time
        # When use_view_dependent_pos=False, these are None and will be handled by slice_gaussian
        if self.use_view_dependent_pos:
            lambda_view = self.get_lambda_view.squeeze(-1)  # [N]
            lambda_time = self.get_lambda_time.squeeze(-1) if self.input_dim == 7 else None  # [N] or None
        else:
            lambda_view = None  # Will use default in slice_gaussian
            lambda_time = None

        # V2: Always compute conditional slicing (no test mode)
        ...
        m_cond, cov3D_precomp, pdf_cond = self.slice_gaussian(cond_params, c_dim=3, lambda_view=lambda_view, lambda_time=lambda_time)
    ...
    def render_tcgs_with_metric(self, viewpoint_camera, background=None, scaling_modifier=1.0,
        ...
        # Get lambda_view
        # When use_view_dependent_pos=False, this is None and will be handled by slice_gaussian
        if self.use_view_dependent_pos:
            lambda_view = self.get_lambda_view.squeeze(-1)  # [N]
        else:
            lambda_view = None  # Will use default in slice_gaussian
        ...
        shs = self.get_features
        m_cond, cov3D_precomp, pdf_cond = self.slice_gaussian(cond_params, c_dim=3, lambda_view=lambda_view)
    ...
    def view_tcgs(self, camera_state, render_tab_state):
        ...
        # Handle lambda parameters only if view-dependent position shift is enabled
        if self.use_view_dependent_pos:
            _lambda_view_masked = self._lambda_view[mask]
        ...
        if self.use_view_dependent_pos:
            orig_lambda_view, self._lambda_view = self._lambda_view, _lambda_view_masked
        ...
        try:
            ...
            if self.use_view_dependent_pos:
                self._lambda_view = orig_lambda_view
