...
#
# Position-based DGS with diagonal-only L_22_inv parameterization
# Simplified version that removes lambda_opc (absorbed into diagonal)
#
# Key simplifications:
# - L_22_inv_diag: [N, C] instead of [N, C*(C+1)/2] (diagonal only, no off-diagonal)
# - No lambda_opc parameter (absorbed into diagonal elements)
# - V_22^{-1} = diag(exp(d_0)^2, ..., exp(d_{C-1})^2)
#
...
# Import CUDA-accelerated diagonal slice function
from gsplat import slice_gaussian_simple_diag
...
class GaussianModel:
    ...
    but uses a simplified diagonal-only precision parameterization:
    - v_12: [N, 3*C] position-view covariance block (C=3 for view, C=4 for view+time)
    - L_22_inv_diag: [N, C] diagonal elements only (no off-diagonal)

    Key insight: V_22^{-1} = diag(exp(d_0)^2, ..., exp(d_{C-1})^2)
    - No matrix multiply needed, just element-wise operations
    - No separate lambda_opc parameter (absorbed into diagonal)

    Conditional position: x_cond = x + v_12 @ V_22^{-1} @ (v - μ_v)
    Opacity scale: exp(-sum_i (v_i - μ_v_i)^2 * exp(d_i)^2)

    Parameter reduction for C=3:
    - L_22_inv: 6 params -> 3 params (50% reduction)
    - lambda_opc: 1 param -> 0 params (removed)
    - Total: 7 -> 3 params per Gaussian for precision/opacity
    """
    ...
    def __init__(self, sh_degree: int, input_dim: int = 6, use_beta: bool = False):
        ...
        # L_22_inv_diag: [N, C] diagonal elements only (activated as exp(d)^2)
    ...
    def slice_gaussian(self, query):
        """
        Perform conditional Gaussian slicing for position using diagonal-only parameterization.
        Uses CUDA-accelerated slice_gaussian_simple_diag.

        Given query view direction (+ optional time), compute:
        - Conditional position mean: x_cond = x + v_12 @ V_22^{-1} @ (v - μ_v)
        - Opacity scale: exp(-sum_i (v_i - μ_v_i)^2 * exp(d_i)^2)

        V_22^{-1} = diag(exp(d_0)^2, ..., exp(d_{C-1})^2) (diagonal precision)
        ...
        """
        x_cond, opacity_scale = slice_gaussian_simple_diag(
            self._xyz,              # [N, 3] - position mean
            self.get_view_mean,     # [N, C] - view direction mean
            query,                  # [N, C] - query view direction
            self.get_v_12,          # [N, 3*C] - position-view covariance (bounded)
            self._L_22_inv_diag,    # [N, C] - diagonal precision parameters
        )
        return x_cond, opacity_scale

    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        """
        Initialize Gaussians from point cloud data with diagonal-only parameterization.
        """
        ...
        # L_22_inv_diag: [N, C] diagonal elements only
        # exp(d)^2 gives precision diagonal
        # Initialize with log(sqrt(2)) ≈ 0.347 so exp(d)^2 ≈ 2.0
    ...
    def construct_list_of_attributes(self):
        ...
        # Simplified parameterization attributes (diagonal-only)
    ...
    def load_ply(self, path):
        ...
        # Load diagonal-only parameterization
        ...
        L_22_inv_diag_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("L_22_inv_diag_")]
    ...
    def render_tcgs(self, viewpoint_camera, render_mode="RGB", scaling_modifier=1.0, use_tcgs=False, tight_snugbox=False, compact_box_mult=1.0):
        ...
        This encapsulates the DGS-specific rendering logic with diagonal-only L_22_inv parameterization.
        ...
        # Compute conditional position and opacity scale using slice_gaussian (diagonal-only)
        m_cond, opacity_scale = self.slice_gaussian(cond_params)
