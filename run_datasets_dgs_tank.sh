...
base_dirs=( # "/code/dataset/tandt_db/rvr_x"
...
for dir in "${base_dirs[@]}" ; do
    ...
    if [ -d "$dir" ]; then
        ...
        clean_dir="${clean_dir#/code/dataset/}"  # Remove the prefix
        echo "$clean_dir"
        python train.py -s "dataset/${clean_dir}" --model_path "output/dgs/${clean_dir}" --eval 
        python render.py -m "output/dgs/${clean_dir}" --skip_train  --iteration 500
        python render.py -m "output/dgs/${clean_dir}" --skip_train  --iteration 2000
        python render.py -m "output/dgs/${clean_dir}" --skip_train  --iteration 7000
        python render.py -m "output/dgs/${clean_dir}" --skip_train  --iteration 15000
        python render.py -m "output/dgs/${clean_dir}" --skip_train  --iteration 30000
        python metrics.py -m "output/dgs/${clean_dir}" # Compute error metrics on renderings 
    fi
done