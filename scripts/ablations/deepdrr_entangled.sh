    ...
    for dir in "$base_dir"*/; do
        if [ -d "$dir" ]; then
            ...
            python train.py -s "../dataset/${clean_dir}" \
                --model_path "../output/ablation2/ddgs_entangled/${clean_dir}" \
                --eval

            # Render at multiple iterations
            for iter in 500 2000 7000 15000 30000; do
                python render.py -m "../output/ablation2/ddgs_entangled/${clean_dir}" \
                    --iteration ${iter} \
                    --skip_train
            done

            # Compute metrics
            python metrics.py -m "../output/ablation2/ddgs_entangled/${clean_dir}"
        fi
    done
