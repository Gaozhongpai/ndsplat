...
base_dirs=("/code/dataset/ct_data/22022107v2")
...
for dir in "${base_dirs[@]}" ; do
    ...
    if [ -d "$dir" ]; then
        ...
        python train.py -s "../dataset/${clean_dir}" --model_path "../output/test/3dgs/${clean_dir}" --eval
        ...
        python render.py -m "../output/test/3dgs/${clean_dir}" --skip_train --iteration 30000
        python metrics.py -m "../output/test/3dgs/${clean_dir}" # Compute error metrics on renderings 
    fi
done
