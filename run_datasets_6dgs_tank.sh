...
base_dirs=("/code/dataset/tandt_db/subsurface_dragon2")
...
for dir in "${base_dirs[@]}" ; do
    ...
    if [ -d "$dir" ]; then
        ...
        clean_dir="${clean_dir#/code/dataset/}"  # Remove the prefix
        echo "$clean_dir"
        python train.py -s "../dataset/${clean_dir}" --model_path "../output/6dgs_rebuttal/${clean_dir}" --eval
        python render.py -m "../output/6dgs_rebuttal/${clean_dir}" --skip_train  --iteration 500
        python render.py -m "../output/6dgs_rebuttal/${clean_dir}" --skip_train  --iteration 2000
        python render.py -m "../output/6dgs_rebuttal/${clean_dir}" --skip_train  --iteration 7000
        python render.py -m "../output/6dgs_rebuttal/${clean_dir}" --skip_train  --iteration 15000
        python render.py -m "../output/6dgs_rebuttal/${clean_dir}" --skip_train  --iteration 30000
        python metrics.py -m "../output/6dgs_rebuttal/${clean_dir}" # Compute error metrics on renderings 
    fi
done