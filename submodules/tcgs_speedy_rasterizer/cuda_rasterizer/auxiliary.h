...
__device__ inline float computeTruncationFactor(float3 pos, float var_xx, float x_threshold) {
    const float EPSILON = 1e-7f;
    float world_dist = x_threshold - pos.x;
    float std_dev = sqrt(max(var_xx, EPSILON));
    float normalized_dist = world_dist / (sqrt(2.0f) * std_dev);
    return 0.5f + 0.5f * erff(normalized_dist);
}

__device__ inline float computeNormalizedDist(float3 pos, float var_xx, float x_threshold) {
    const float EPSILON = 1e-7f;
    float world_dist = x_threshold - pos.x;
    float std_dev = sqrt(max(var_xx, EPSILON));
    return world_dist / (sqrt(2.0f) * std_dev);
}

__device__ inline float computeTruncationGradients(
...
{
    const float EPSILON = 1e-7f;
    const float std_dev = sqrt(max(var_xx, EPSILON));
    const float exp_term = exp(max(-80.0f, -normalized_dist * normalized_dist / 2.0f));
    const float dtrunc_dnorm = exp_term / sqrtf(6.28318530717958647692f);
    *dalpha_dx = -base_alpha * dtrunc_dnorm / (sqrt(2.0f) * std_dev + EPSILON);
    *dalpha_dvar = base_alpha * dtrunc_dnorm * normalized_dist / (2.0f * (var_xx + EPSILON));
    return dtrunc_dnorm;
}

__forceinline__ __device__ bool in_frustum_with_cutting_plane(
    int idx,
    const float* orig_points,
    const float* viewmatrix,
    const float* projmatrix,
...
{
    const float EPSILON = 1e-7f;
    p_view = make_float3(0.0f, 0.0f, 0.0f);

    float3 p_orig = { orig_points[3 * idx], orig_points[3 * idx + 1], orig_points[3 * idx + 2] };
    p_view = transformPoint4x3(p_orig, viewmatrix);

    float std_dev_x = sqrt(max(cov3D[idx * 6 + 0], EPSILON));
    if (p_orig.x - 3 * std_dev_x > x_threshold)
    {
        if (prefiltered)
        {
            printf("Point is filtered although prefiltered is set. This shouldn't happen!");
